    ...
    <div class="container">
        <div class="row">
            <div class="col-md-9 py-3">
                ...
                $query = "SELECT * FROM posts WHERE subredditId IN (SELECT subId FROM subscriptions WHERE userId = '$id') ORDER BY id DESC LIMIT 20";
                $q = mysqli_query($db, $query);

                if (mysqli_num_rows($q) > 0) {
                    while ($row = mysqli_fetch_assoc($q)) {
                        $subID = $row['subredditId'];
                        $q2 = "SELECT * FROM subreddit WHERE id = '$subID'";
